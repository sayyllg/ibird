Ibird.module('Components', function(Components, Ibird, Backbone, Marionette, $, _){
	Components.Dicts = {};
	...
	*/
	...
	initialize: function(options) {
	  ...
	  //组件share 判空补丁
	  Ibird.Components.Form.validators.required = function(options) {
	    options = _.extend({
	      type: 'required',
	      message: Ibird.Components.Form.validators.errMessages.required
	    }, options);
	...
	// Collection: Ibird.Entities.Collection,
	...
	*/
	...
	defaults:{
	  multiple: true,     // 多文件上传
	  params:{app_type:'Ibirds_app_spms',app_id:'project_tmp_id'},      // 上传文件参数 如app_type/app_id等
	...
	uploadComplete: function(uniqueId, name, resp, status, xhr) {
	  ...
	  if( resp['file_type']=='Ibirds_n_image') fileinfo.thumb = resp.append.thumbnails.link;
	...
	*/
	...
	initialize:function(options){
	  _.extend(options);
	  var url = !options['schema']['url'] ? Ibird_HOSTS+'/cmri/suggestion/share' : Ibird_HOSTS+options['schema']['url'];
	...
	Collection: Ibird.Entities.ShareDepts,
	onRender: function(){
	    var self = this;
	    this.constructor.__super__.onRender.apply(this, arguments);
	    this.$el.on('beforeItemRemove', function(evt){
	...
	*/
	...
	// datalist start
	Components.Pills = Ibird.Views.List.extend({
		className: 'nav nav-pills'
	});

	Components.FilterItem = Ibird.Views.Item.extend({
		onClick:function(e){
	...
	Components.Filter = Components.Pills.extend({
		...
		emptyView: Ibird.Views.ItemView
	});

	Components.GroupItem = Ibird.Views.Item.extend({
		className: 'list-group-item'
	});

	Components.ListGroup = Ibird.Views.List.extend({
		className: 'list-group',
		childView: Components.GroupItem
	});

	Components.Pagination = Ibird.Views.List.extend({
	...
	Components.DataList = Ibird.Views.Layout.extend({
		className: 'data-list',
		template: Ibird.Templates.datalist,
		FilterView: Components.Filter,
		ListView: Components.ListGroup,
		PaginationView: Components.Pagination,
		initialize: function(options) {
		  var collection = new Ibird.Entities.Collection(options.filter||[]);
		  this.filter = new this.FilterView({collection:collection});
		  this.list = new this.ListView({collection: this.collection});
		  this.pagination = new this.PaginationView({collection: this.collection,model:options.filterModel||new Ibird.Entities.Model()});
		},
		onShow: function() {
